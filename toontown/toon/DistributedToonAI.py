...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def shouldToonUp(self, zoneId):
        if zoneId == OTPGlobals.QuietZone:
            return None
        elif ToontownGlobals.safeZoneCountMap.has_key(ZoneUtil.getBranchZone(zoneId)):
            return True
        else:
    ...
    def reqCogSummons(self, type, suitIndex):
        if type not in ('single', 'building', 'invasion'):
            ...
            self.sendUpdate('cogSummonsResponse', ['fail', suitIndex, 0])
            return
        elif not self.hasCogSummons(suitIndex, type):
            self.air.writeServerEvent('suspicious', avId=self.doId, issue='bogus cog summons')
            self.sendUpdate('cogSummonsResponse', ['fail', suitIndex, 0])
            return
        elif ZoneUtil.isWelcomeValley(self.zoneId):
            self.sendUpdate('cogSummonsResponse', ['fail', suitIndex, 0])
            return
        else:
            returnCode = None
...
def setTrackAccess(toonup, trap, lure, sound, throw, squirt, drop):
    ...
    spellbook.getTarget().b_setTrackAccess([toonup,
     trap,
     lure,
     sound,
     throw,
...
def maxToon(hasConfirmed = 'UNCONFIRMED', missingTrack = None):
    ...
    if hasConfirmed != 'CONFIRM':
        ...
        toon.b_setCogParts([CogDisguiseGlobals.PartsPerSuitBitmasks[0],
...
@magicWord(category=CATEGORY_SYSADMIN, types=[str, int, int])
def inventory(a, b = None, c = None):
    ...
    if a == 'reset':
        ...
        if not -1 <= targetTrack < len(ToontownBattleGlobals.Tracks):
            return 'Invalid target track index: {0}'.format(targetTrack)
        elif targetTrack != -1 and not av.hasTrackAccess(targetTrack):
...
def dna(part, value):
    ...
    if part == 'headColor':
        ...
        dna.headColor = ToonDNA.colorToInt[value]
    elif part == 'armColor':
        value = value.title()
        if value not in ToonDNA.colorToInt.keys():
            return 'DNA: Invalid color specified for arms.'
        dna.armColor = ToonDNA.colorToInt[value]
    elif part == 'legColor':
        value = value.title()
        if value not in ToonDNA.colorToInt.keys():
            return 'DNA: Invalid color specified for legs.'
        dna.legColor = ToonDNA.colorToInt[value]
    elif part == 'color':
        value = value.title()
        if value not in ToonDNA.colorToInt.keys():
...
def unites(amt = 999):
    ...
    spellbook.getTarget().restockAllResistanceMessages(amt)
...
def trackBonus(track):
    ...
    if track == 'all':
        ...
        pass
    elif track.isdigit() and 0 <= int(track) < numTracks:
...
@magicWord(category=CATEGORY_OVERRIDE)
def fanfare():
    """ Give target toon a fanfare because why not? """
...
def emotes():
    """
    Unlock all of the emotes on the target toon.
    """
    av = spellbook.getTarget()
...
def phrase(phraseStringOrId):
    ...
    elif av.customMessages.count(id) != 0:
