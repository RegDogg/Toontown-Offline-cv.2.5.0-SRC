...
class DistributedPartyManagerAI(DistributedObjectAI):
    ...
    def addPartyRequest(self, hostId, startTime, endTime, isPrivate, inviteTheme, activities, decorations, inviteeIds):
        if hostId != simbase.air.getAvatarIdFromSender():
            ...
            return
        self.notify.info('Party requested: Host: %s, Start: %s, End: %s, Private: %s, Invite Theme: %s, Activities omitted, Decor omitted, Invitees: %s' % (hostId,
         startTime,
         endTime,
         isPrivate,
         inviteTheme,
    ...
    def getPublicParties(self):
        ...
        for partyId in self.pubPartyInfo:
            ...
            if guests > 255:
                ...
                guests = 0
            p.append([party['shardId'],
             party['zoneId'],
             guests,
             party.get('hostName', ''),
             party.get('activities', []),
             minLeft])
